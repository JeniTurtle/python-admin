...
class Model(dict):
    ...
    def select_by(cls, where, condition=[], fields=[]):
        ...
        L = database.select('select %s from `%s` %s' % (fields, cls.__table__, where), *condition)
        return [cls(**d) for d in L]
    ...
    def join_by(cls, source_list, source_field, target_field, where=''):
        ...
        L = database.select('select * from `%s` %s' % (cls.__table__, where))
    ...
    def count_by_field(cls, where, field, *args):
        ...
        """
        return database.select_int('select count(%s) from `%s` %s' % (field, cls.__table__, where), *args)
        ...
        for k, v in self.__mappings__.iteritems():
            if v.updatable:
                if hasattr(self, k):
                    ...
                    args.append(arg)
        args.extend(params)
        database.update('update `%s` set %s %s' % (self.__table__, ','.join(L), where), *args)
        return self
...
if __name__ == '__main__':
    ...
    database.create_engine(configs['user'], configs['password'], configs['database'], configs['host'])
    database.update('drop table if exists test')
    database.update('create table test (id int primary key, name text, email text, passwd text, last_modified real)')
    import doctest
    doctest.testmod()
